...
class Pinger(object):
    ...
    def dequeue(self):
        while True:
            ...
            self.status_list.append("{},{},{}".format(timestamp,ip,result))
    ...
    def start(self):
        ...
        result = (os.linesep.join(ping.status_list))
        ping.status_list = []
        return result
...
if __name__ == '__main__':
    ...
    delay_seconds = 1
    total_seconds = 10
    total_iterations = total_seconds / delay_seconds
    with open("out.csv", 'w') as out:
        ...
        for iteration in range(total_iterations):
            ...
            ping.thread_count = len(ping.hosts)
            info = ping.start()
            out.write (info + os.linesep)
            timestamp = datetime.datetime.now().strftime('%Y-%m-%d,%H:%M:%S')
            print ("{}: ping {}/{} done, sleeping {}...".format(timestamp, iteration+1, total_iterations, delay_seconds))
